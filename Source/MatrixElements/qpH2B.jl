...
function qpH1B_HFB(Params::Parameters,SQE::pnVector)
    # Read parameters ...
    N_max = Params.Calc.Nmax
    N_2max = 2 * N_max
    a_max = div((N_max + 1)*(N_max + 2),2)
    ...
    # Allocate H_N
    H_N = qpH1B(diagm(SQE),pnMatrix(zeros(Float64,a_max,a_max),zeros(Float64,a_max,a_max)))
...
# WORK IN PROGRESS ...
function qpH_export(Params::Parameters,Orb::Vector{NOrb},Orb_NN::NNOrb,SQE::pnVector,H_NN::qpH2B)
    # Read parameters ...
    N_max = Params.Calc.Nmax
    N_2max = 2*N_max
