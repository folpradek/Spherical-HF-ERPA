...
function OBDM_radial_summary(Params::Parameters,Summary_File::String,pR2::Float64,nR2::Float64,chR2::Float64,R_CMS::Vector{Float64},kR2::Float64)
    ...
    Summary =  open(Summary_File, "a")
        ...
        @printf(Summary, "\nr_p   = %12.6f fm \t\t ... \t Point-proton radius\n", sqrt(pR2 + R_CMS[1] + R_CMS[2] + 0.5 * (197.326980 / 938.272013)^2))
        @printf(Summary, "r_n   = %12.6f fm \t\t ... \t Point-neutron radius\n", sqrt(nR2 + R_CMS[3] + R_CMS[4] + 0.5 * (197.326980 / 939.565346)^2))
        @printf(Summary, "r_ch  = %12.6f fm \t\t ... \t Standard charge radius\n", sqrt(pR2 + R_CMS[1] + R_CMS[2] - 0.106 * N / Z + 0.8414 - 0.143))
        @printf(Summary, "r_ch  = %12.6f fm \t\t ... \t Folded charge radius\n", sqrt(chR2 + R_CMS[1] + R_CMS[2] + 0.5 * (197.326980 / 938.272013)^2))
        ...
        @printf(Summary, "\nr_p   = %12.6f fm \t\t ... \t Point-proton radius\n", sqrt(pR2 + R_CMS[1] + R_CMS[2] + 0.5 * (197.326980 / 938.272013)^2) / A^(1/3))
        @printf(Summary, "r_n   = %12.6f fm \t\t ... \t Point-neutron radius\n", sqrt(nR2 + R_CMS[3] + R_CMS[4] + 0.5 * (197.326980 / 939.565346)^2) / A^(1/3))
        @printf(Summary, "r_ch  = %12.6f fm \t\t ... \t Standard charge radius\n", sqrt(pR2 + R_CMS[1] + R_CMS[2] - 0.106 * N / Z + 0.8414 - 0.143) / A^(1/3))
        @printf(Summary, "r_ch  = %12.6f fm \t\t ... \t Folded charge radius\n", sqrt(chR2 + R_CMS[1] + R_CMS[2] + 0.5 * (197.326980 / 938.272013)^2) / A^(1/3))
        @printf(Summary, "\nr_p  = %12.6f fm \t\t ... \t Uncorrected point-proton radius\n", sqrt(pR2) / A^(1/3))
        @printf(Summary, "r_ch = %12.6f fm \t\t ... \t Uncorrected folded charge radius\n", sqrt(chR2) / A^(1/3))
        @printf(Summary, "r_n  = %12.6f fm \t\t ... \t Uncorrected point-neutron radius\n", sqrt(nR2) / A^(1/3))
        println(Summary, "\nDetails on radii corrections ...\n")
