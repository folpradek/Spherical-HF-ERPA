...
function Tr1b_E1_vortical_initialize1(Params::Parameters,Orb::Vector{Orb1B},chR2::Float64)
    # Read parameters
    hw = Params.Calc.hw
    N_max = Params.Calc.Nmax
    a_max = div((N_max + 1)*(N_max+2),2)
    ...
    @inline function rME_YL_cross_nabla_Omega(l_a::Int64,l_b::Int64,L::Int64,J::Int64)
        ...
        @inbounds for l in max(abs(L - l_a), abs(1 - l_b)):min(L + l_a, 1 + l_b)
            if rem(l_a + l + L, 2) == 0 && rem(l + l_b + 1, 2) == 0 #&& abs(L - l) <= l_a && abs(1 - l_b) <= l && l_a <= (L + l) && l <= (1 + l_b)
    ...
    @inline function rME_nabla_dot_rN_YJL(a::Int64,b::Int64,N::Int64,L::Int64,J::Int64,hw::Float64,Orb::Vector{Orb1B})
        ...
        elseif J == 1 && L == 0 && N == 0
            rME = phase(l_a + div(j_b + 3,2)) * sqrt(Float64((j_a + 1) * (j_b + 1))) * f6j(j_a,2*l_a,1,2*l_b,j_b,2) *
                 (rME_rN_dr(n_a,l_a,n_b,l_b,0,hw) * rME_n_cross_YL(l_a,l_b,0,1) + rME_rN(n_a,l_a,n_b,l_b,-1,hw) * rME_nabla_Omega_cross_YL(l_a,l_b,0,1))
            return rME
        else
            return 0.0
        end
    ...
    @inline function rME_nabla_cross_S_dot_rN_YJL(a::Int64,b::Int64,N::Int64,L::Int64,J::Int64,hw::Float64,Orb::Vector{Orb1B})
        ...
        elseif J == 1 && L == 0 && N == 0
            rME = 3.0 * sqrt(3.0 / 2.0 * Float64((j_a + 1) * (j_b + 1))) * f9j(2*l_a,1,j_a,2*l_b,1,j_b,2,2,2) *
                 (rME_rN_dr(n_a,l_a,n_b,l_b,N,hw) * rME_n_cross_YL(l_a,l_b,0,1) + rME_rN(n_a,l_a,n_b,l_b,-1,hw) * rME_nabla_Omega_cross_YL(l_a,l_b,0,1))
            return rME
        else
            return 0.0
        end
    ...
    @inline function rME_nabla_dot_rN_YJL(a::Int64,b::Int64,N::Int64,L::Int64,J::Int64,hw::Float64,Orb::Vector{Orb1B})
        ...
        if N != 0
            rME = phase(l_a + J + div(j_b + 1,2)) *  sqrt(Float64((j_a + 1) * (j_b + 1))) * f6j(2*l_b,j_b,1,j_a,2*l_a,2*J) *
                  ((rME_rN_dr(n_a,l_a,n_b,l_b,N,hw) * rME_n_cross_YL(l_a,l_b,L,J) + phase(l_b - l_a) * rME_rN_dr(n_b,l_b,n_a,l_a,N,hw) * rME_n_cross_YL(l_b,l_a,L,J)) * phase(1 + L - J) +
                   (rME_rN(n_a,l_a,n_b,l_b,N-1,hw) * rME_YL_cross_nabla_Omega(l_a,l_b,L,J) + phase(l_b - l_a) * rME_rN(n_b,l_b,n_a,l_a,N-1,hw) * rME_YL_cross_nabla_Omega(l_b,l_a,L,J)))
