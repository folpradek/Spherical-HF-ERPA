                ...
                @inbounds for Ket in 1:Bra
                    ...
                    if Bra <= N_T0
                        ...
                        @inbounds for I in 0:J_max
                            ...
                            2*I <= (j_c + j_b) && (rem(l_a + l_d,2) + 1 == P) && (rem(l_b + l_c,2) + 1 == P)
                                Amp = Float64((-1)^(J+1) * (2*I + 1)) * f6j(j_a,j_b,2*J,j_c,j_d,2*I)
                                Bra_ad, Ket_cb = V2b_temp_index(a,d,I,P,Orb_NN_t), V2b_temp_index(c,b,I,P,Orb_NN_t)
                                Hpn2002Sum += Amp * V_NN_t2.pn2002[P,I+1][Bra_ad,Ket_cb]
                                Hpn1111Sum += Amp * V_NN_t2.pn1111[2][P,I+1][Bra_ad,Ket_cb]
                                Hpn0220Sum += Amp * V_NN_t2.pn0220[P,I+1][Bra_ad,Ket_cb]
                            end
                        end
                    ...
                    if Bra <= N_T1
                        ...
                        @inbounds for I in 0:J_max
                            if abs(j_a - j_d) <= 2*I && 2*I <= (j_a + j_d) && abs(j_c - j_b) <= 2*I &&
                            2*I <= (j_c + j_b) && (rem(l_a + l_d,2) + 1 == P) && (rem(l_b + l_c,2) + 1 == P)
                                Amp11 = 4.0 * Float64((-1)^(J) * (2*I + 1)) * f6j(j_a,j_b,2*J,j_c,j_d,2*I)
                                Bra_ad, Ket_cb = V2b_temp_index(a,d,I,P,Orb_NN_t), V2b_temp_index(c,b,I,P,Orb_NN_t)
                                HppSum += Amp11 * V_NN_t2.pp[2][P,I+1][Bra_ad,Ket_cb]
                                HnnSum += Amp11 * V_NN_t2.nn[2][P,I+1][Bra_ad,Ket_cb]
                            end
                        end
