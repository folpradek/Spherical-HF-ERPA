function HF_RPA_export(Params::Parameters,Orb::Vector{Orb1B},N_nu::Matrix{Int64},E_RPA_Corr::Float64,E_TDA::Matrix{Vector{Float64}},E_RPA::Matrix{Vector{ComplexF64}},X_RPA::Matrix{Matrix{ComplexF64}},Y_RPA::Matrix{Matrix{ComplexF64}},CI_TDA::Matrix{Vector{Float64}},CI_RPA::Matrix{Vector{Float64}},rB_TDA::ReducedTransition,rB_RPA::ReducedTransition,Rho_RPA::O1B,C_HF::O1B)

    # Export of RPA & TDA summary ...
    @time HF_RPA_summary(Params,N_nu,E_RPA_Corr)
...
function HF_RPA_summary(Params::Parameters,N_nu::Matrix{Int64},E_RPA_Corr::Float64)
    # Read parameters ...
    A = Params.Calc.A
    Z = Params.Calc.Z
    hw = Params.Calc.hw
    ...
    Output_Path = "IO/" * Output_File * "/RPA/HF_RPA_Summary.dat"

    println("\nPreparing HF-RPA & HF-TDA summary ...")
    Summary =  open(Output_Path, "w")
        println(Summary, "Spherical Hartree-Fock Random-Phase Approximation & Tamm-Dancoff Approximation review:")
        ...
        println(Summary, "\nE_corr = " * string(round(E_RPA_Corr, digits = 8)) * "\t MeV \t...\tRPA ground-state correlation energy")
        println(Summary, "\nE_corr / A = " * string(round(E_RPA_Corr / Float64(A), digits = 8)) * "\t MeV / Nucleon \t...\tRPA ground-state correlation energy per nucleon")
    close(Summary)
...
function HF_RPA_amplitudes_export(Params::Parameters,N_nu::Matrix{Int64},E_RPA::Matrix{Vector{ComplexF64}},X_RPA::Matrix{Matrix{ComplexF64}},Y_RPA::Matrix{Matrix{ComplexF64}})
    ...
    # Set export paths ...
    Output_Path = "IO/" * Output_File * "/RPA/Amplitudes/HF_RPA_Amplitudes_Ordered.dat"
    ...
    # Set export path ...
    Output_Path = "IO/" * Output_File * "/RPA/Amplitudes/HF_RPA_Amplitudes.dat"
