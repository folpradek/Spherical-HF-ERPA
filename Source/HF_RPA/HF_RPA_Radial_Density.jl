function HF_RPA_Radial_Density(Params::Vector{Any},Orb::Vector{NOrb},pRho::Matrix{Float64},nRho::Matrix{Float64})
    ...
    if CMS == "CMS1+2B" || CMS == "CMS2B"
        R_CMS = HF_RPA_Radial_CMS(Params,Orb,pU,nU,pRho,nRho)
    else
        R_CMS = [0.0, 0.0, 0.0, 0.0]
    end

    # Evaluate HF densities on a grid ...
    Rho_Grid = HF_RPA_Radial_Density_Grid(Params,Orb,pU,nU,pRho,nRho)

    # Evaluate HF charged density on a grid & calculte anomalous magnetic moment correction ...
    Rho_Grid, kR2 = HF_RPA_Radial_ChDensity_Grid(Params,Orb,pU,nU,pRho,nRho,R_CMS,Rho_Grid[1],Rho_Grid[2],Rho_Grid[3])
...
function HF_RPA_Radial_CMS(Params::Vector{Any},Orb::Vector{NOrb},pU::Matrix{Float64},nU::Matrix{Float64},pRho::Matrix{Float64},nRho::Matrix{Float64})
    # Load parameters ...
    A = Params[1]
    Z = Params[2]
    HbarOmega = Params[3]
...
function HF_RPA_Radial_Density_Grid(Params::Vector{Any},Orb::Vector{NOrb},pU::Matrix{Float64},nU::Matrix{Float64},pRho::Matrix{Float64},nRho::Matrix{Float64})
    # Basic constants ...
    HbarC = 197.326980
    m_n = 939.565346
    m_p = 938.272013
...
function HF_RPA_Radial_ChDensity_Grid(Params::Vector{Any},Orb::Vector{NOrb},pU::Matrix{Float64},nU::Matrix{Float64},pRho::Matrix{Float64},nRho::Matrix{Float64},R_CMS::Vector{Float64},r_grid::Vector{Float64},pRho_rad::Vector{Float64},nRho_rad::Vector{Float64})
    # Basic constants ...
    HbarC = 197.326980
    m_n = 939.565346
    m_p = 938.272013
...
function HF_RPA_Radial_Export(Params::Vector{Any},Rho_Grid::Vector{Vector{Float64}})
    # Read parameters ...
    Output_File = Params[8]

    # Export r_grid radius & pRho, nRho, chRho HF-RPA densities ...
    Density_File_name = Output_File * "/RPA/Densities/HF_RPA_Radial_Densities.dat"
...
function HF_RPA_Radial_Summary(Params::Vector{Any},pR2::Float64,nR2::Float64,chR2::Float64,R_CMS::Vector{Float64},kR2::Float64)
    # Read parameters ...
    Orthogon = Params[5]
    Output_Path = Params[8]

    if Orthogon == true
        Output_File = Output_Path * "/RPA/HF_RPA_Summary_Ortho.dat"
    else
        Output_File = Output_Path * "/RPA/HF_RPA_Summary_Spur.dat"
    end

    println("\nResulting point-proton, charged & point-neutron radii ...")
    println("\nr_p  = " * string(round(sqrt(pR2 + R_CMS[1] + R_CMS[2] + 0.5 * (197.326980 / 938.272013)^2), sigdigits=6)) * " fm \t\t ... \t RPA ground state point proton radius")
    println("r_ch = " * string(round(sqrt(chR2 + R_CMS[1] + R_CMS[2] + 0.5 * (197.326980 / 938.272013)^2), sigdigits=6)) * " fm \t\t ... \t RPA ground state charged radius")
    println("r_n  = " * string(round(sqrt(nR2 + R_CMS[3] + R_CMS[4] + 0.5 * (197.326980 / 939.565346)^2), sigdigits=6)) * " fm \t\t ... \t RPA ground state point neutron radius")
    println("\nFor more details see HF_RPA_Summary.dat file ...")

    # Export resulsts & contributions to radii ...
    println("\nExporting information on HF_RPA radii ...")

    Summary_File =  open(Output_File, "a")
        println(Summary_File, "\nInformation on HF-RPA radii:")
        println(Summary_File, "\nr_p  = " * string(round(sqrt(pR2 + R_CMS[1] + R_CMS[2] + 0.5 * (197.326980 / 938.272013)^2), sigdigits=6)) * " fm \t\t ... \t RPA ground state point proton radius")
        println(Summary_File, "r_ch = " * string(round(sqrt(chR2 + R_CMS[1] + R_CMS[2] + 0.5 * (197.326980 / 938.272013)^2), sigdigits=6)) * " fm \t\t ... \t RPA ground state charged radius")
        println(Summary_File, "r_n  = " * string(round(sqrt(nR2 + R_CMS[3] + R_CMS[4] + 0.5 * (197.326980 / 939.565346)^2), sigdigits=6)) * " fm \t\t ... \t RPA ground state point neutron radius")
    ...
    println("\nHF-RPA radial densities and radii exported ...")
end