function HF_RPA_Radial_Density(Params::Parameters,Orb::Vector{NOrb},Rho::pnMatrix)
    ...
    if CMS == "CMS1+2B" || CMS == "CMS2B"
        R_CMS = HF_RPA_Radial_CMS(Params,Orb,U,Rho)
    else
...
function HF_RPA_Radial_CMS(Params::Parameters,Orb::Vector{NOrb},U::pnMatrix,Rho::pnMatrix)
    # Load parameters ...
    A = Params.Calc.A
    Z = Params.Calc.Z
...
function HF_RPA_Radial_Density_Grid(Params::Parameters,Orb::Vector{NOrb},U::pnMatrix,Rho::pnMatrix)
    # Basic constants ...
    HbarC = 197.326980
    m_n = 939.565346
    m_p = 938.272013
...
function HF_RPA_Radial_Summary(Params::Parameters,pR2::Float64,nR2::Float64,chR2::Float64,R_CMS::Vector{Float64},kR2::Float64)
    # Read parameters ...
    Orthogon = Params.Calc.Ortho
    Output_Path = Params.Calc.Path

    if Orthogon == true
        Output_File = "IO/" * Output_Path * "/RPA/HF_RPA_Summary_Ortho.dat"
    else
        Output_File = "IO/" * Output_Path * "/RPA/HF_RPA_Summary_Spur.dat"
    end
