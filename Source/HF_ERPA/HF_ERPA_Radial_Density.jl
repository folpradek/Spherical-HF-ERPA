function HF_ERPA_Radial_Density(Params::Parameters,Orb::Vector{NOrb},Rho::pnMatrix,U::pnMatrix)
    ...
    if CMS == "CMS1+2B" || CMS == "CMS2B"
        R_CMS = HF_ERPA_Radial_CMS(Params,Orb,U,Rho)
    else
        R_CMS = [0.0, 0.0, 0.0, 0.0]
    end

    # Evaluate HF densities on a grid ...
    Rho_Grid = HF_ERPA_Radial_Density_Grid(Params,Orb,U,Rho)

    # Evaluate HF charged density on a grid & calculte anomalous magnetic moment correction ...
    Rho_Grid, kR2 = HF_ERPA_Radial_ChDensity_Grid(Params,Orb,U,Rho,R_CMS,Rho_Grid[1],Rho_Grid[2],Rho_Grid[3])
...
function HF_ERPA_Radial_CMS(Params::Parameters,Orb::Vector{NOrb},U::pnMatrix,Rho::pnMatrix)
    # Load parameters ...
    A = Params.Calc.A
    Z = Params.Calc.Z
...
function HF_ERPA_Radial_Density_Grid(Params::Parameters,Orb::Vector{NOrb},U::pnMatrix,Rho::pnMatrix)
    # Basic constants ...
    HbarC = 197.326980
    m_n = 939.565346
    m_p = 938.272013
...
function HF_ERPA_Radial_ChDensity_Grid(Params::Parameters,Orb::Vector{NOrb},U::pnMatrix,Rho::pnMatrix,R_CMS::Vector{Float64},r_grid::Vector{Float64},pRho_rad::Vector{Float64},nRho_rad::Vector{Float64})
    # Basic constants ...
    HbarC = 197.326980
    m_n = 939.565346
    m_p = 938.272013
...
function HF_ERPA_Radial_Export(Params::Parameters,Rho_Grid::Vector{Vector{Float64}})
    # Read parameters ...
    Output_File = Params.Calc.Path

    # Export r_grid radius & pRho, nRho, chRho HF mean-field densities ...
    Density_File_name = "IO/" * Output_File * "/ERPA/Densities/HF_ERPA_Radial_Densities.dat"
...
function HF_ERPA_Radial_Summary(Params::Parameters,pR2::Float64,nR2::Float64,chR2::Float64,R_CMS::Vector{Float64},kR2::Float64)
    # Read parameters ...
    Orthogon = Params.Calc.Ortho
    Output_Path = Params.Calc.Path

    if Orthogon == true
        Output_File = "IO/" * Output_Path * "/ERPA/HF_ERPA_Summary_Ortho.dat"
    else
        Output_File = "IO/" * Output_Path * "/ERPA/HF_ERPA_Summary_Spur.dat"
    end
