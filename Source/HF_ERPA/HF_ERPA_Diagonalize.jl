function HF_ERPA_0_Diagonalize(Params::Vector{Any},A::Matrix{Matrix{Float64}},B::Matrix{Matrix{Float64}},N_nu::Matrix{Int64})
...
function HF_ERPA_I_Diagonalize(Params::Vector{Any},A::Matrix{Matrix{Float64}},B::Matrix{Matrix{Float64}},N_nu::Matrix{Int64})
    # Read calculation parameters ...
    N_max = Params[4]
    N_2max = 2*N_max
    J_max = N_2max + 1
...
function HF_ERPA_Diagonalize(Params::Vector{Any},A::Matrix{Matrix{Float64}},B::Matrix{Matrix{Float64}},N_nu::Matrix{Int64},Orb_Phonon::Matrix{Vector{Int64}},Phonon::Vector{PhState},Particle::pnSVector,Hole::pnSVector,TrOp::TranOper,pRho::Matrix{Float64},nRho::Matrix{Float64},X_RPA_0::Matrix{Matrix{ComplexF64}},Y_RPA_0::Matrix{Matrix{ComplexF64}})
    # Read calculation parameters ...
    N_max = Params[4]
    N_2max = 2*N_max
    J_max = N_2max + 1
    ...
    @inbounds for JP in JP_List
        ...
        if (Orthogon == true) && (J == 1) && (P == 2)
            Spur_State = HF_ERPA_Spur_Ini(N_ph,Orb_Phonon[J+1,P],Phonon,Particle,Hole,TrOp,pRho,nRho,X_RPA_0,Y_RPA_0)
