function HF_ERPA_0_Allocate(Params::Parameters,N_nu::Matrix{Int64},Orb_Phonon::Matrix{Vector{Int64}},Phonon::Vector{PhState},Particle::pnSVector,Hole::pnSVector,Orb::Vector{NOrb},Orb_NN::NNOrb,VNN::NNInt)
...
function HF_ERPA_Allocate(Params::Parameters,N_nu::Matrix{Int64},Orb_Phonon::Matrix{Vector{Int64}},Phonon::Vector{PhState},Particle::pnSVector,Hole::pnSVector,Orb::Vector{NOrb},Orb_NN::NNOrb,VNN::NNInt,Rho::pnMatrix,H::pnMatrix)
    # Read calculation parameters ...
    N_max = Params.Calc.Nmax
    N_2max = 2*N_max
    J_max = N_2max + 1
    ...
    @inbounds Threads.@threads for JP in JP_List
        ...
        @inbounds for ind_ph in 1:N_ph
            ...
            @inbounds for ind_qg in 1:N_ph
                ...
                if t_ph == t_qg
                    if t_ph == -1
                        Amp = 0.5 * (sqrt((Rho.p[a_h,a_h] - Rho.p[a_p,a_p]) / (Rho.p[a_g,a_g] - Rho.p[a_q,a_q])) + sqrt((Rho.p[a_g,a_g] - Rho.p[a_q,a_q]) / (Rho.p[a_h,a_h] - Rho.p[a_p,a_p]))) * (H.p[a_p,a_q] * KroneckerDelta(a_h,a_g) - H.p[a_h,a_g] * KroneckerDelta(a_p,a_q))
                        ASum += Amp
                    elseif t_ph == 1
                        Amp = 0.5 * (sqrt((Rho.n[a_h,a_h] - Rho.n[a_p,a_p]) / (Rho.n[a_g,a_g] - Rho.n[a_q,a_q])) + sqrt((Rho.n[a_g,a_g] - Rho.n[a_q,a_q]) / (Rho.n[a_h,a_h] - Rho.n[a_p,a_p]))) * (H.n[a_p,a_q] * KroneckerDelta(a_h,a_g) - H.n[a_h,a_g] * KroneckerDelta(a_p,a_q))
                        ASum += Amp
                    end
                end
                    ...
                    @inbounds for J_r in div(abs(j_p - j_g),2):div((j_p + j_g),2)
                        if (abs(j_q - j_h) <= 2*J_r) && (2*J_r <= (j_q + j_h))
                            if t_ph == -1 && t_qg == -1
                                ...
                                phase_A = (-1)^(div(j_h + j_q,2) + J_r)
                                ME = Amp * phase_A * (2*J_r + 1) * f6j(j_p, j_h, 2*J, j_q, j_g, 2*J_r) * V2B(a_p,a_g,a_h,a_q,J_r,1,VNN.pp,Orb,Orb_NN)
                                ASum +=  ME
                            elseif t_ph == -1 && t_qg == 1
                                Amp = sqrt((Rho.p[a_h,a_h] - Rho.p[a_p,a_p]) * (Rho.n[a_g,a_g] - Rho.n[a_q,a_q]))
                                phase_A = (-1)^(div((j_h + j_q),2) + J_r)
                                ME = Amp * phase_A * (2*J_r + 1) * f6j(j_p, j_h, 2*J, j_q, j_g, 2*J_r) * V2B(a_p,a_g,a_h,a_q,J_r,0,VNN.pn,Orb,Orb_NN)
                                ASum += ME
                            elseif t_ph == 1 && t_qg == -1
                                Amp = sqrt((Rho.n[a_h,a_h] - Rho.n[a_p,a_p]) * (Rho.p[a_g,a_g] - Rho.p[a_q,a_q]))
                                phase_A = (-1)^(div((j_g + j_p),2) + J_r)
                                ME = Amp * phase_A * (2*J_r + 1) * f6j(j_q, j_g, 2*J, j_p, j_h, 2*J_r) * V2B(a_q,a_h,a_g,a_p,J_r,0,VNN.pn,Orb,Orb_NN)
                                ASum += ME
                            elseif t_ph == 1 && t_qg == 1
                                Amp = sqrt((Rho.n[a_h,a_h] - Rho.n[a_p,a_p]) * (Rho.n[a_g,a_g] - Rho.n[a_q,a_q]))
                                phase_A = (-1)^(div((j_h + j_q),2) + J_r)
                                ME = Amp * phase_A * (2*J_r + 1) * f6j(j_p, j_h, 2*J, j_q, j_g, 2*J_r) * V2B(a_p,a_g,a_h,a_q,J_r,1,VNN.nn,Orb,Orb_NN)
                                ASum +=  ME
                            end
                        end
                    end
                    ...
                    @inbounds for J_r in div(abs(j_p - j_q),2):div((j_p + j_q),2)
                        if (abs(j_h - j_g) <= 2*J_r) && (2*J_r <= (j_h + j_g))
                            ...
                            if t_ph == -1 && t_qg == -1
                                Amp = sqrt((Rho.p[a_h,a_h] - Rho.p[a_p,a_p]) * (Rho.p[a_g,a_g] - Rho.p[a_q,a_q]))
                                Phase = Float64((-1)^(div(j_h + j_q,2) + J + J_r))
                                ME = Amp * Phase * Float64(2*J_r + 1) * f6j(j_p,j_h,2*J,j_g,j_q,2*J_r) * V2B(a_p,a_q,a_h,a_g,J_r,1,VNN.pp,Orb,Orb_NN)
                                BSum += ME
                            elseif t_ph == -1 && t_qg == 1
                                Amp = sqrt((Rho.p[a_h,a_h] - Rho.p[a_p,a_p]) * (Rho.n[a_g,a_g] - Rho.n[a_q,a_q]))
                                Phase = Float64((-1)^(div(j_h + j_q,2) + J + J_r))
                                ME = Amp * Phase * (2*J_r + 1) * f6j(j_p,j_h,2*J,j_g,j_q,2*J_r) * V2B(a_p,a_q,a_h,a_g,J_r,0,VNN.pn,Orb,Orb_NN)
                                BSum += ME
                            elseif t_ph == 1 && t_qg == -1
                                Amp = sqrt((Rho.n[a_h,a_h] - Rho.n[a_p,a_p]) * (Rho.p[a_g,a_g] - Rho.p[a_q,a_q]))
                                Phase = Float64((-1)^(div(j_g + j_p,2) + J + J_r))
                                ME = Amp * Phase * (2*J_r + 1) * f6j(j_q,j_g,2*J,j_h,j_p,2*J_r) * V2B(a_q,a_p,a_g,a_h,J_r,0,VNN.pn,Orb,Orb_NN)
                                BSum += ME
                            elseif t_ph == 1 && t_qg == 1
                                Amp = sqrt((Rho.n[a_h,a_h] - Rho.n[a_p,a_p]) * (Rho.n[a_g,a_g] - Rho.n[a_q,a_q]))
                                Phase = Float64((-1)^(div(j_h + j_q,2) + J + J_r))
                                ME = Amp * Phase * (2*J_r + 1) * f6j(j_p,j_h,2*J,j_g,j_q,2*J_r) * V2B(a_p,a_q,a_h,a_g,J_r,1,VNN.nn,Orb,Orb_NN)
                                BSum += ME
                            end
