function HFB_Density_Operator_Initialize(Params::Parameters,Orb::Vector{NOrb})
    ...
    pa0, na0 = Params.Calc.Pairing.pdN0, Params.Calc.Pairing.ndN0
    ...
    Z, N = HFB_Particle_Number(Params,pnMatrix(pRho,nRho),Orb)
            ...
            if (Orb[a].j == Orb[b].j) && (Orb[a].l == Orb[b].l)
                ...
                @inbounds for c in 1:a_max
                    if (Orb[a].j == Orb[c].j) && (Orb[a].l == Orb[c].l)
                        pMERho, pMEKappa = V.p[a,c] * V.p[b,c], U.p[a,c] * V.p[b,c]
                        nMERho, nMEKappa = V.n[a,c] * V.n[b,c], U.n[a,c] * V.n[b,c]
