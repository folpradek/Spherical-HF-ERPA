function HFB_Solver(Params::Parameters)
	...
	# Comment this one to include allocation & export of 1-body + 2-body quasiparticle Hamiltonian ...
	# expressed in the canonical HFB particle basis (C) & HFB quasiparticle basis (U, V) ...
    return
...
function HFB_Solve(Params::Parameters,Orb::Vector{NOrb},Orb_NN::NNOrb,Orb_NNN::NNNOrb,T::Matrix{Float64},VNN::NNInt,VNNN::Array{Vector{Vector{Float32}},4},epsilon::Float64)
    ...
    while ((dE > epsilon) || (dZ > epsilon) || (dN > epsilon)) && (Iteration < Iteration_max)
        ...
        if d2E < dE * 1e-3
            println("\nHFB iteration stucked at a degenerate solution ... Iteration termines & the last solution will be choosen ...\n")
            break
        end
