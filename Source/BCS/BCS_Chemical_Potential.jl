function BCS_Lambda(Params::Parameters,SPE::pnVector,Delta::pnVector,Lambda::pnFloat,dA::pnFloat,Orb::Vector{NOrb})
    # Read parameters ...
    ...
    if abs(dA.p) > 1e-8
        dZ = dA.p
        @inbounds for iteration in 1:500
            pLambda = pLambda + 0.05 * dZ
            Z = 0.0
...
function HF_BCS_Initialize_Chemical_Potential(Params::Parameters,SPE::pnVector,Orb::Vector{NOrb})
    # Read parameters ...
    N_max = Params.Calc.Nmax
    a_max = div((N_max + 1) * (N_max + 2), 2)
