function BCS_Energy(Params::Parameters,Rho::pnMatrix,Kappa::pnMatrix,Orb::Vector{NOrb},Orb_NN::NNOrb,Orb_NNN::NNNOrb,T::Matrix{Float64},VNN::NNInt,VNNN::Array{Vector{Vector{Float32}},4})
    # Read parameters ...
    A = Params.Calc.A
    ...
    E_MF_partial = Threads.Atomic{Float64}[Threads.Atomic{Float64}(0.0) for _ in 1:Threads.nthreads()]
    ...
    @inbounds Threads.@threads for a in 1:a_max
        ...
        @inbounds for b = 1:a_max
                            ...
                            if (l_b == l_e && j_b == j_e) && ((2*(n_d + n_e) + l_d + l_e) <= N_2max)
                                ...
                                @inbounds for J = div(abs(j_a - j_b),2):div((j_a + j_b),2)
                                    ...
                                    if (rem(l_a + l_b, 2) == rem(l_d + l_e, 2))
                                        ...
                                        Hat =  Float64(2*J + 1)
                                        @views E_MF_partial[thread_id][] += 0.5 * Hat * pRho_ad * pRho_be * V2B(a,b,d,e,J,1,VNN.pp,Orb,Orb_NN)
                                        @views E_MF_partial[thread_id][] += 0.5 * Hat * nRho_ad * nRho_be * V2B(a,b,d,e,J,1,VNN.nn,Orb,Orb_NN)
                                        @views E_MF_partial[thread_id][] += Hat * pRho_ad * nRho_be * V2B(a,b,d,e,J,0,VNN.pn,Orb,Orb_NN)
                                        ...
                                        if ((2*(n_a + n_b + n_c) + l_a + l_b + l_c) <= N_3max)
                                            ...
                                            @inbounds for f = 1:a_max
                                                ...
                                                if (2*(n_d + n_e + n_f) + l_d + l_e + l_f) <= N_3max && l_c == l_f && j_c == j_f && P == (rem(l_d + l_e + l_f, 2)+1)
                                                    ...
                                                    @views E_MF_partial[thread_id][] += 1.0/6.0 * (me5 * pRho_ad * pRho_be * pRho_cf + (2.0 * me4 + me5) *
            ...
            if CMS == "CMS1+2B"
                @views E_MF_partial[thread_id][] += (Rho.p[a,b] + Rho.n[a,b]) * T[a,b] * Float64(j_a + 1) * (1.0 - 1.0 / Float64(A))
            elseif CMS == "CMS2B"
                @views E_MF_partial[thread_id][] += 0
            else
                @views E_MF_partial[thread_id][] += (Rho.p[a,b] + Rho.n[a,b]) * T[a,b] * Float64(j_a + 1)
            end
        end
    end
                        ...
                        @inbounds for e = 1:a_max
                            ...
                            if ((2*(n_d + n_e) + l_d + l_e) <= N_2max) && l_d == l_e
                                ...
                                if j_d == j_e
                                    ...
                                    # 2-body NN interaction ...
                                    @views E_BCS_partial[thread_id][] += 0.25 * j_a_hat * j_d_hat * pKappa_ab * pKappa_de * V2B(a,b,d,e,0,1,VNN.pp,Orb,Orb_NN)
                                    @views E_BCS_partial[thread_id][] += 0.25 * j_a_hat * j_d_hat * nKappa_ab * nKappa_de * V2B(a,b,d,e,0,1,VNN.nn,Orb,Orb_NN)
    ...
    E_MF += sum(x[] for x in E_MF_partial)
    E_BCS += sum(x[] for x in E_BCS_partial)

    println("\nMF energy     ...   E_MF  = " * string(E_MF) * " MeV")
    println("\nBCS energy    ...   E_BCS = " * string(E_BCS) * " MeV")
