function BCS_V2B_Res(Params::Parameters,Orb::Vector{NOrb},Orb_NN::NNOrb,Orb_NNN::NNNOrb,VNN::NNInt,VNNN::Array{Vector{Vector{Float32}},4},C::pnMatrix,Rho::pnMatrix)
    ...
    # Read the pairing rescaling coefficients s2 & s3 ...
        ...
        @inbounds Threads.@threads for Bra in 1:max(N_t0, N_t1)
            ...
            if Bra <= N_t0
                ...
                for Ket in 1:Bra
                    ...
                    @views VNN_res.pn[P,J+1][Ind] = s2 * VNN_res.pn[P,J+1][Ind] + s3 * pnSum
                end
            end
            ...
            if Bra <= N_t1
                ...
                for Ket in 1:Bra
                    ...
                    @views VNN_res.pp[P,J+1][Ind] = s2 * VNN_res.pp[P,J+1][Ind] + s3 * ppSum
                    @views VNN_res.nn[P,J+1][Ind] = s2 * VNN_res.nn[P,J+1][Ind] + s3 * nnSum
                end
