function pHF_RPA_Allocate(Params::Parameters,N_nu::Matrix{Int64},Orb_Phonon::Matrix{Vector{Int64}},Phonon::Vector{PhState},Particle::pnSVector,Hole::pnSVector,Orb::Vector{NOrb},Orb_NN::NNOrb,VNN::NNInt)
                ...
                if rem(l_p + l_q, 2) == rem(l_h + l_g, 2)
                    @inbounds for J_r in div(abs(j_p - j_q),2):div((j_p + j_q),2)
                        if (abs(j_h - j_g) <= 2*J_r) && (2*J_r <= (j_h + j_g))
                            ...
                            if t_ph == -1 && t_qg == -1
                                Phase = Float64((-1)^(div(j_h + j_q,2) + J + J_r))
                                ME = Phase * Float64(2*J_r + 1) * f6j(j_p,j_h,2*J,j_g,j_q,2*J_r) * V2B(a_p,a_q,a_h,a_g,J_r,1,VNN.pp,Orb,Orb_NN) / (E_h + E_g - E_p - E_q)
                                MSum += ME
                            elseif t_ph == -1 && t_qg == 1
                                Phase = Float64((-1)^(div(j_h + j_q,2) + J + J_r))
                                ME = Phase * (2*J_r + 1) * f6j(j_p,j_h,2*J,j_g,j_q,2*J_r) * V2B(a_p,a_q,a_h,a_g,J_r,0,VNN.pn,Orb,Orb_NN) / (E_h + E_g - E_p - E_q)
                                MSum += ME
                            elseif t_ph == 1 && t_qg == -1
                                Phase = Float64((-1)^(div(j_g + j_p,2) + J + J_r))
                                ME = Phase * (2*J_r + 1) * f6j(j_q,j_g,2*J,j_h,j_p,2*J_r) * V2B(a_q,a_p,a_g,a_h,J_r,0,VNN.pn,Orb,Orb_NN) / (E_h + E_g - E_p - E_q)
                                MSum += ME
                            elseif t_ph == 1 && t_qg == 1
                                Phase = Float64((-1)^(div(j_h + j_q,2) + J + J_r))
                                ME = Phase * (2*J_r + 1) * f6j(j_p,j_h,2*J,j_g,j_q,2*J_r) * V2B(a_p,a_q,a_h,a_g,J_r,1,VNN.nn,Orb,Orb_NN) / (E_h + E_g - E_p - E_q)
                                MSum += ME
                            end
