function HF_RRPA_export(Params::Parameters,Orb::Vector{Orb1B},N_nu::Matrix{Int64},E_Corr::Float64,E_RPA::Matrix{Vector{ComplexF64}},X_RPA::Matrix{Matrix{ComplexF64}},Y_RPA::Matrix{Matrix{ComplexF64}},CI_RPA::Matrix{Vector{Float64}},rB_RPA::ReducedTransition,Rho::O1B,C::O1B)
    # Export RRPA summary file ...
    HF_RRPA_summary(Params,N_nu,E_Corr)

    # Export of RRPA spectra ...
    HF_RRPA_spectrum_export(Params,N_nu,E_RPA,CI_RPA)
    ...
    # Export of RRPA electric transitions ...
    HF_RRPA_transition_export(Params,N_nu,E_RPA,rB_RPA)

    # Export dimensions of phonon subspaces ...
    HF_RRPA_phonon_space_export(Params,N_nu)

    return
end

function HF_RRPA_summary(Params::Parameters,N_nu::Matrix{Int64},E_0::Float64)
    # Read parameters ...
    A = Params.Calc.A
    Z = Params.Calc.Z
    hw = Params.Calc.hw
    ...
    # Set the path for "HF_RRPA_Summary.dat" output file ...
    Output_Path = "IO/" * Output_File * "/RRPA/HF_RRPA_Summary.dat"
    
    println("\nPreparing HF-RRPA summary ...")
    Summary =  open(Output_Path, "w")
        println(Summary, "Spherical Hartree-Fock Renormalized-Random-Phase Approximation review:")
        ...
        println(Summary, "\nE_corr = " * string(round(E_0, digits = 8)) * "\t MeV \t...\tRRPA ground-state correlation energy")
        println(Summary, "\nE_corr / A = " * string(round(E_0 / Float64(A), digits = 8)) * "\t MeV / Nucleon \t...\tRRPA ground-state correlation energy per nucleon")
...
function HF_RRPA_spectrum_export(Params::Parameters,N_nu::Matrix{Int64},E_RPA::Matrix{Vector{ComplexF64}},CI_RPA::Matrix{Vector{Float64}})
    ...
    # Export name & path ...
    Output_Path_RPA = "IO/" * Output_File * "/RRPA/Spectra/RRPA.dat"

    println("\nPreparing RRPA spectra export ...")
...
function HF_RRPA_spectrum_export_plot(Params::Parameters,N_nu::Matrix{Int64},E_RPA::Matrix{Vector{ComplexF64}})
    ...
    # Export name & path ...
    Output_Path_RPA = "IO/" * Output_File * "/RRPA/Spectra/RRPA_Plot.dat"
    ...
    println("\nRRPA plot-ready spectra exported ...")
...
function HF_RRPA_occupation_export(Params::Parameters,Orb::Vector{Orb1B},Rho::O1B)
    ...
    # RRPA occupations export ....
    open(Output_Path, "w") do Write_File
        println(Write_File, "a\ta_p\tpN_HF\tpN_RRPA\tpN_dep\ta_n\tnN_HF\tnN_RRPA\tnN_dep")
        @inbounds for a in 1:a_max
            println(Write_File, string(a) * "\t" * string(pa[a]) * "\t" * string(pO_HF[a]) * "\t" * string(pO_RRPA[a]) * "\t" * string(round(pO_RRPA[a] - pO_HF[a], digits = 5)) * "\t" * string(na[a]) * "\t" * string(nO_HF[a]) * "\t" * string(nO_RRPA[a]) * "\t" * string(round(nO_RRPA[a] - nO_HF[a], digits = 5)))
        end
    end
...
function HF_RRPA_amplitudes_export(Params::Parameters,N_nu::Matrix{Int64},E_RPA::Matrix{Vector{ComplexF64}},X_RPA::Matrix{Matrix{ComplexF64}},Y_RPA::Matrix{Matrix{ComplexF64}})
    ...
    # Export path ...
    Output_Path = "IO/" * Output_File * "/RRPA/Amplitudes/HF_RRPA_Amplitudes_Ordered.dat"

    println("\nPreparing export of RRPA amplitudes X & Y ...")
    ...
    println("\nExported norms of RRPA amplitudes |X|^2 & |Y|^2 ...")

    # Allocate new export path ...
    Output_File = Params.Calc.Path

    Output_Path = "IO/" * Output_File * "/RRPA/Amplitudes/HF_RRPA_Amplitudes.dat"
...
function HF_RRPA_transition_export(Params::Parameters,N_nu::Matrix{Int64},E_RPA::Matrix{Vector{ComplexF64}},rB_RPA::ReducedTransition)
    # Read parameters ...
    Orthogon = Params.Calc.RRPA.Ortho
    ...
    println("\nRRPA transition intensities succesfully exported ...")

    return
end

function HF_RRPA_phonon_space_export(Params::Parameters,N_nu::Matrix{Int64})
    # Read parameters ...
    N_max = Params.Calc.Nmax
    N_2max = 2*N_max
    J_max = N_2max + 1
    ...
    println("\nPreparing export of dimensions of RRPA phonon subspaces ...")

    # Output file name & path ...
    Output_Path = "IO/" * Output_File * "/RRPA/HF_RRPA_Phonon_Subspace_Size.dat"
    ...
    println("\nDimensions of RRPA phonon subspaces exported ...")
